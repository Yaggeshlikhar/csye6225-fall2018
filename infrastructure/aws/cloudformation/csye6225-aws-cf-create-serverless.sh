...
create=$(aws cloudformation create-stack --stack-name $stackName --template-body file://csye6225-aws-cf-serverless.json --capabilities CAPABILITY_NAMED_IAM \
  --parameters  ParameterKey=S3Key,ParameterValue=lambdafunction.zip  ParameterKey=LambdaRole,ParameterValue=$Role  ParameterKey=domain,ParameterValue=${aws_domain_name:0:-1} )
